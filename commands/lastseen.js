...
exports.run = async (client, message, args, level) => { 
  ...
  if (!singleTarget) {
    searchObj.members.forEach(function (target, targetID, mapObj){
      if (targetID != process.env.DISCORD_BOT_ID) {
        targetDB = targetDB = client.userDB.get(targetID) || {username: `<@${targetID}>`}
        if (targetDB.lastSeen) {
          hasData=true;
          let timeDiff = Math.round((Date.now() - targetDB.lastSeen) / 3600000);
          scoreTable.cell('User', targetDB.username);
          let lastSeen = timeDiff ? `${timeDiff} hours ago` : "just now...";
  ...
  if (targetDB.timeOffset)
    ...
    message.reply(`${targetDB.username} was last seen ${timeDiff} hours ago`);
